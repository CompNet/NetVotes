...
#############################################################################################
load.external.partition <- function(f.name, algo.name)
{	if(startsWith(algo.name,COMDET.ALGO.ILS) || startsWith(algo.name,COMDET.ALGO.GRASP))
		result <- load.mestrado.partition(f.name, algo.name)
...
{	# identify the latest folder
	...
	folder.name <- file.path(part.folder,net.fname,temp.folders[length(temp.folders)])
