...
#############################################################################################
plot.lines <- function(plot.file, series, axes=list(), graph.title, with.lines=TRUE, consider.stdev=TRUE, format=c("PDF","PNG"))
{	# complete axes lists (if needed)
	...
	for(frmt in format)
	{	# set plot file
		plot.file <- paste(plot.file,".",format,sep="")
		# create the file
		if(!is.na(frmt))
		{	if(frmt=="PNG")
		...
		title(main=graph.title, 
				xlab=axes$x$title, ylab=axes$y$title);
